...
def test_apply_evaluation_records_the_gap_implication_on_a_terminal_outcome(tmp_path):
    ...
    item = roadmap_schema.find_item(roadmap_schema.load_roadmap(path), item_id)
    assert item["gap_implication"] == "mechanism-inapplicable"


def test_a_retry_records_no_gap_implication(tmp_path):
    # A PUSH_BACK has not finished proving anything about the bottleneck.
    path = _write(tmp_path, VALID)
    item_id = VALID["items"][0]["id"]
    roadmap_schema.apply_evaluation(
        path,
    ...
    item = roadmap_schema.find_item(roadmap_schema.load_roadmap(path), item_id)
    assert "gap_implication" not in item
