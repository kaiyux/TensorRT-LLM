...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        "budget unless the roadmap exhausts with no outstanding measurement request "
        "or the improvement target is met, verify the final state with one "
        "independent QA benchmark, reconcile the final theoretical model offline "
        "with the same analyzer, and report "
        "measured gains and the remaining gap in performance_model.yaml — via a "
        "benchmarker -> [optional profiler -> analyzer -> "
        "(optimizer <-> evaluator) items -> optional integrator] x rounds "
        "-> qa -> final_analyzer -> reporter loop. "
        "Final reconciliation must produce nonempty analysis.md and a validated "
        "performance_model.yaml before reporting. "
    ...
    parser.add_argument(
        ...
        "schema-validated kernel_ledger.yaml each round. Independently of "
        "kernel coverage or the projector, every analyzer turn updates "
        "performance_model.yaml: the best current theoretical performance model, "
        "matching measurements at every operating point, "
    ...
    parser.add_argument(
        ...
        "sol_projection.md, baseline/, tuning/, rounds/ with analysis.md, "
        "performance_model.yaml, profiler_report.md and optional kernel_ledger.yaml, "
        "final_verification/ with QA evidence and the reconciled analysis/model, "
    ...
    parser.add_argument(
        ...
        "progress.yaml, baseline/, rounds/, final_verification/, worktrees/, tuning/, sol_work/, "
    ...
    parser.add_argument(
        ...
        "(+ sol_work/), current performance model, and newest analysis "
        "(+ traces and "
        "kernel_ledger.yaml) are copied into this workspace, the "
        "benchmarker/projector stages are skipped, and round 1's analyzer "
        "runs plan-only by default — writing analysis.md, performance_model.yaml "
        "and roadmap.yaml from the imported evidence "
        "with no server, profiler, or benchmark. The corrected source model is "
        "read-only reused_analysis/prior_performance_model.yaml; a new current model "
        "is still required. A source roadmap.yaml is "
    ...
    parser.add_argument(
        "--reanalyze",
        action="store_true",
        help="With --reuse-analysis DIR, regenerate round 1's analysis.md and "
