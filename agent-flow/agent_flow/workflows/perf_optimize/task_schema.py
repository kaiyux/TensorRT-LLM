...
It also honors a perf-optimize-only key inside the shared ``profile``
...
- ``profile.kernel_coverage`` — optional; when present (an empty mapping
  ...
  ``profile.methods``.
  Every row also references a best theoretical performance model with
  predictions, matching measurements, unexplained discrepancies, and
  evidence-backed revision history. Every analyzer turn updates this
  single ledger, including replan-only rounds. The SOL projector is
